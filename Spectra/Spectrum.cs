...
namespace Spectra
{
        ...
        where TPeak : MZPeak
    {
        ...
        public virtual TPeak GetClosestPeak(double mean)
        {
        ...
        public virtual Spectrum<TPeak> CorrectMasses(Func<double, double> convertor)
        {
            Spectrum<TPeak> newSpectrum = Clone();
            for (int i = 0; i < newSpectrum.Count; i++)
                newSpectrum.Masses[i] = convertor(newSpectrum.Masses[i]);
            return newSpectrum;
        }

        public virtual Spectrum<TPeak> Extract(IRange<double> mzRange)
        {
            return Extract(mzRange.Minimum, mzRange.Maximum);
        }

        public virtual Spectrum<TPeak> FilterByMZ(IRange<double> mzRange)
        {
            return FilterByMZ(mzRange.Minimum, mzRange.Maximum);
        }

        public virtual Spectrum<TPeak> FilterByIntensity(IRange<double> intensityRange)
        {
        ...
        public abstract Spectrum<TPeak> Extract(double minMZ, double maxMZ);
        ...
        /// </summary>
        /// <returns></returns>
        public abstract Spectrum<TPeak> Clone();
        ...
        protected int GetClosestPeakIndex(double meanMZ)
        {
            if (Count == 0)
                return -1;
